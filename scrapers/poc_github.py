...
class PoCGitHubScraper(BaseScraper):
    ...
    def _repo_record(self, cve: Optional[str], repo: dict) -> VulnRecord:
        fn = repo.get("full_name", "")
        return VulnRecord(
            cve=cve, id=f"POCGH:{fn}", title=fn,
            source=self.name, url=repo.get("html_url", ""),
            severity=None, cvss=None, description=repo.get("description", "") or "",
            affected=[], poc_refs=[repo.get("html_url")] if repo.get("html_url") else [],
            published=None, raw={"stars": repo.get("stargazers_count")},
        )
